...
resource "aws_autoscaling_group" "realtechie-autoscaling" {
  ...
  tag {
    key                 = "Name"
    value               = "realtechie Custom EC2 instance"
...
resource "aws_cloudwatch_metric_alarm" "realtechie-cpu-alarm" {
  alarm_name          = "realtechie-cpu-alarm"
  ...
  dimensions = {
    "AutoScalingGroupName" = aws_autoscaling_group.realtechie-autoscaling.name
  }

  actions_enabled = true
  alarm_actions   = [aws_autoscaling_policy.realtechie-cpu-policy.arn]
}

#Auto Descaling Policy
resource "aws_autoscaling_policy" "realtechie-cpu-policy-scaledown" {
  name                   = "realtechie-cpu-policy-scaledown"
  autoscaling_group_name = aws_autoscaling_group.realtechie-autoscaling.name
  adjustment_type        = "ChangeInCapacity"
  scaling_adjustment     = "-1"
  cooldown               = "200"
...
resource "aws_cloudwatch_metric_alarm" "realtechie-cpu-alarm-scaledown" {
  alarm_name          = "realtechie-cpu-alarm-scaledown"
  ...
  actions_enabled = true
  alarm_actions   = [aws_autoscaling_policy.realtechie-cpu-policy-scaledown.arn]
}